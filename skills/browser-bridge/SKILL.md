...
27. **Scroll container auto-detection** - `bbx scroll` and `page.get_text` automatically detect nested scroll containers (e.g. `<main>` on LinkedIn) when the body has `maxY: 0`. When auto-detection fires, `viewport.scroll` returns `autoDetected: true` and `containerTag` in the response. `page.get_state` reports the detected container in `scroll.container` (tag, scrollHeight, clientHeight). `page.get_text` falls back to the container's text when `body.innerText` is empty or very short, returning a `source` field naming the container tag. For sites where auto-detection doesn't find the right element, target it explicitly: `bbx call viewport.scroll '{"target":{"selector":"main"},"top":1500}'`.
...
- Artifact delivery returns a descriptor (artifactId, sha256, byteLength), not bytes. From MCP, read chunks with the `browser_artifact` tool (`action: "read"`, paging `offset` until `nextOffset` is null, then verify the reassembled bytes against `sha256`) and free storage with `action: "delete"`. Each standalone CLI invocation has a new artifact owner, so use `bbx screenshot` / `bbx har` to capture, download, verify, and delete within one CLI process; use `delivery: "inline"` for raw CLI screenshot calls that need the bytes in their immediate result.
