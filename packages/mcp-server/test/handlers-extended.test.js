...
function fail(code, message) {
  return /** @type {import('../../protocol/src/types.js').BridgeResponse} */ ({
    id: 'req_test', ok: false, result: null,
...
test('handleTabsTool close calls tabs.close with tabId', async () => {
  const { handleTabsTool } = await import('../src/handlers.js');
  await withMockedBridge(async () => ok({ closed: true, tabId: 5 }), async (calls) => {
...
test('handleDomTool text calls dom.get_text with textBudget', async () => {
  await withMockedBridge(async (record) => {
    if (record.method === 'dom.query') {
      return ok({ nodes: [{ elementRef: 'el_1', tag: 'p', attrs: {}, bbox: {}, textExcerpt: '' }] });
    }
    return ok({ value: 'Hello world', truncated: false, length: 11 });
  }, async (calls) => {
    const result = await handleDomTool({ action: 'text', selector: 'p', budgetPreset: 'quick' });
...
test('handleDomTool html calls dom.get_html', async () => {
  await withMockedBridge(async (record) => {
    if (record.method === 'dom.query') {
      return ok({ nodes: [{ elementRef: 'el_1', tag: 'div', attrs: {}, bbox: {}, textExcerpt: '' }] });
    }
    return ok({ html: '<div>hi</div>', truncated: false });
  }, async (calls) => {
    const result = await handleDomTool({ action: 'html', selector: 'div', budgetPreset: 'normal' });
...
test('handleDomTool describe calls dom.describe', async () => {
  await withMockedBridge(async (record) => {
    ...
    return ok({ tag: 'div', elementRef: 'el_1', bbox: { width: 100, height: 50 } });
  }, async (calls) => {
    const result = await handleDomTool({ action: 'describe', selector: 'div' });
...
test('handleDomTool wait calls dom.wait_for', async () => {
  await withMockedBridge(async () => ok({ found: true, duration: 100, elementRef: 'el_1' }), async (calls) => {
    const result = await handleDomTool({ action: 'wait', selector: '.loading', state: 'visible' });
...
test('handleDomTool find_text calls dom.find_by_text', async () => {
  await withMockedBridge(async () => ok({
    nodes: [{ elementRef: 'el_1', tag: 'span', attrs: {}, bbox: {}, textExcerpt: 'target' }]
  }), async (calls) => {
    const result = await handleDomTool({ action: 'find_text', text: 'target' });
...
test('handleDomTool find_role calls dom.find_by_role', async () => {
  await withMockedBridge(async () => ok({
    nodes: [{ elementRef: 'el_1', tag: 'button', attrs: {}, bbox: {}, textExcerpt: 'OK' }]
  }), async (calls) => {
    const result = await handleDomTool({ action: 'find_role', role: 'button' });
...
test('handleDomTool returns error for unsupported action', async () => {
  await withMockedBridge(async () => ok({}), async () => {
...
test('handleDomTool query infers quick budget for ID selectors', async () => {
  await withMockedBridge(async () => ok({
    nodes: [{ elementRef: 'el_1', tag: 'div', attrs: {}, bbox: {}, textExcerpt: '' }]
  }), async (calls) => {
...
test('handlePageTool console calls page.get_console with budget preset', async () => {
  await withMockedBridge(async () => ok({
    ...
    const result = await handlePageTool({ action: 'console', budgetPreset: 'quick' });
...
test('handlePageTool network calls page.get_network with budget preset', async () => {
  await withMockedBridge(async () => ok({
    ...
    const result = await handlePageTool({ action: 'network', budgetPreset: 'deep' });
...
test('handlePageTool storage calls page.get_storage', async () => {
  await withMockedBridge(async () => ok({
    ...
    const result = await handlePageTool({ action: 'storage', type: 'localStorage' });
...
test('handlePageTool wait_for_load calls page.wait_for_load_state', async () => {
  await withMockedBridge(async () => ok({}), async (calls) => {
    const result = await handlePageTool({ action: 'wait_for_load', timeoutMs: 5000 });
...
test('handlePageTool performance calls performance.get_metrics', async () => {
  await withMockedBridge(async () => ok({ metrics: { FCP: 1200 } }), async (calls) => {
...
test('handleNavigationTool resize calls viewport.resize', async () => {
  await withMockedBridge(async () => ok({ resized: true, width: 800, height: 600 }), async (calls) => {
    const result = await handleNavigationTool({ action: 'resize', width: 800, height: 600 });
...
test('handleInputTool type calls input.type', async () => {
  await withMockedBridge(async (record) => {
    if (record.method === 'dom.query') {
      return ok({ nodes: [{ elementRef: 'el_input', tag: 'input', attrs: {}, bbox: {}, textExcerpt: '' }] });
    }
    return ok({ typed: true, elementRef: 'el_input' });
  }, async (calls) => {
    const result = await handleInputTool({ action: 'type', selector: 'input', text: 'hello', clear: true });
...
test('handleInputTool focus calls input.focus', async () => {
  await withMockedBridge(async (record) => {
    ...
    const result = await handleInputTool({ action: 'focus', selector: 'input' });
...
test('handleInputTool press_key calls input.press_key without ref when no selector', async () => {
  await withMockedBridge(async () => ok({ pressed: true, key: 'Enter' }), async (calls) => {
    const result = await handleInputTool({ action: 'press_key', key: 'Enter' });
...
test('handleInputTool hover calls input.hover', async () => {
  await withMockedBridge(async (record) => {
    ...
    const result = await handleInputTool({ action: 'hover', selector: 'div' });
...
test('handleInputTool set_checked calls input.set_checked', async () => {
  await withMockedBridge(async (record) => {
    if (record.method === 'dom.query') {
      return ok({ nodes: [{ elementRef: 'el_cb', tag: 'input', attrs: {}, bbox: {}, textExcerpt: '' }] });
    }
    return ok({});
  }, async (calls) => {
    const result = await handleInputTool({ action: 'set_checked', selector: 'input[type=checkbox]', checked: true });
...
test('handleInputTool select_option calls input.select_option', async () => {
  await withMockedBridge(async (record) => {
    if (record.method === 'dom.query') {
      return ok({ nodes: [{ elementRef: 'el_sel', tag: 'select', attrs: {}, bbox: {}, textExcerpt: '' }] });
    }
    return ok({});
  }, async (calls) => {
    const result = await handleInputTool({ action: 'select_option', selector: 'select', values: ['opt1'] });
...
test('handleInputTool drag calls input.drag with source and destination', async () => {
  await withMockedBridge(async (record) => {
    if (record.method === 'dom.query') {
      return ok({ nodes: [{ elementRef: 'el_src', tag: 'div', attrs: {}, bbox: {}, textExcerpt: '' }] });
    }
    return ok({ dragged: true, sourceRef: 'el_src', destinationRef: 'el_dst' });
  }, async (calls) => {
...
test('handleInputTool scroll_into_view calls input.scroll_into_view', async () => {
  await withMockedBridge(async (record) => {
    ...
    const result = await handleInputTool({ action: 'scroll_into_view', selector: 'div' });
...
test('handlePatchTool rollback calls patch.rollback', async () => {
  await withMockedBridge(async () => ok({ rolledBack: true }), async (calls) => {
    const result = await handlePatchTool({ action: 'rollback', patchId: 'p_1' });
...
test('handleStylesLayoutTool matched_rules calls styles.get_matched_rules', async () => {
  await withMockedBridge(async (record) => {
    ...
    const result = await handleStylesLayoutTool({ action: 'matched_rules', selector: 'div' });
...
test('handleStylesLayoutTool box_model calls layout.get_box_model', async () => {
  await withMockedBridge(async (record) => {
    ...
    return ok({ content: { x: 0, y: 0, width: 100, height: 50 }, border: {} });
  }, async (calls) => {
    const result = await handleStylesLayoutTool({ action: 'box_model', selector: 'div' });
...
test('handleStylesLayoutTool hit_test calls layout.hit_test', async () => {
  await withMockedBridge(async () => ok({ x: 100, y: 200, width: 50, height: 30 }), async (calls) => {
    const result = await handleStylesLayoutTool({ action: 'hit_test', x: 100, y: 200 });
...
test('handleCaptureTool region calls screenshot.capture_region', async () => {
  await withMockedBridge(async () => ok({ image: 'data:image/png;base64,abc', rect: {} }), async (calls) => {
    const result = await handleCaptureTool({ action: 'region', rect: { x: 0, y: 0, width: 100, height: 100 } });
...
test('handleCaptureTool full_page calls screenshot.capture_full_page', async () => {
  await withMockedBridge(async () => ok({ image: 'data:image/png;base64,abc' }), async (calls) => {
...
test('handleRawCallTool calls bridge method and returns result on success', async () => {
  await withMockedBridge(async () => ok({ nodes: [] }), async (calls) => {
    const result = await handleRawCallTool({ method: 'dom.query', params: { selector: 'body' } });
...
test('handleRawCallTool returns error for bridge failure', async () => {
  await withMockedBridge(async () => fail('ACCESS_DENIED', 'Denied'), async () => {
    const result = await handleRawCallTool({ method: 'dom.query', params: {} });
    assert.equal(result.isError, true);
  });
});
