...
type AgentRequestMessage = { type: 'agent.request'; request: { id: string; method: string } };
type AgentResponseMessage = {
  type: 'agent.response';
  response: BridgeResponse;
};
type RegisteredMessage = { type: 'registered'; role: string; clientId: string };
type MockDaemon = { server: Server; port: number };
...
function isRegisteredMessage(value: unknown): value is RegisteredMessage {
  return isRecord(value) && value.type === 'registered';
}

function isAgentResponseMessage(value: unknown): value is AgentResponseMessage {
  return isRecord(value) && value.type === 'agent.response' && isRecord(value.response);
}

function isAgentRequestMessage(value: unknown): value is AgentRequestMessage {
  return (
    isRecord(value) &&
...
test('BridgeClient.request rejects on timeout and removes the waiting entry', async (t) => {
  ...
  t.mock.method(globalThis, 'setTimeout', ((callback: TimerHandler) => {
    queueMicrotask(() => {
  ...
  await assert.rejects(client.request({ method: 'health.ping', timeoutMs: 25 }), (error) => {
    ...
    assert.equal((error as ErrorWithCode).code, 'BRIDGE_TIMEOUT');
...
function ok(result: unknown): BridgeResponse {
  return {
    id: 'req_test',
    ok: true,
    result,
...
test('summarizer: element describe with object text', () => {
  ...
  assert.equal((s.evidence as EvidenceRecord).text, 'Page Title');
});
...
test('summarizer: DOM query evidence includes textExcerpt and attrs', () => {
  ...
  const evidence = s.evidence as EvidenceRecord[];
...
test('summarizer: network URLs are truncated', () => {
  ...
  const evidence = s.evidence as EvidenceRecord[];
  assert.ok(String(evidence[0].url).length <= 130);
});
...
test('interactiveConfirm honors defaults and closes the readline interface', async () => {
  ...
  readline.createInterface = (() =>
    ...
    }) as ReturnType<typeof readline.createInterface>) as typeof readline.createInterface;
...
async function startMockDaemon(
  onRequest?: (socket: Socket, msg: AgentRequestMessage) => void
): Promise<MockDaemon> {
  return new Promise<MockDaemon>((resolve) => {
 ...
 *
 */
function makeTcpClient(port: number): BridgeClient {
  const client = new BridgeClient({ defaultTimeoutMs: 5_000 });
  async function connectOverTcp(this: BridgeClient): Promise<void> {
  ...
  client.connect = connectOverTcp.bind(client);
  return client;
}

test('BridgeClient pending request rejects when server destroys connection mid-request', async () => {
  // Mock daemon that immediately closes the connection when it receives unknown request.
...
test('BridgeClient reconnects and emits reconnected event after server drops connection', async () => {
  ...
  async function reconnectingTcpConnect(this: BridgeClient): Promise<void> {
  ...
  client.connect = reconnectingTcpConnect.bind(client);
