...
type DetectModule = typeof import('../src/detect.js');
type LoadDetectOptions = {
  homeDir: string;
  existingPaths?: string[];
  availableCommands?: string[];
};
type LoadedDetect = {
  detectMcpClients: () => Promise<string[]>;
  detectSkillTargets: () => Promise<string[]>;
  commandChecks: string[];
};
type PlatformDetectOptions = {
...
type LoadedPlatformDetect = DetectModule & {
  commandChecks: string[];
  readdirStarts: Map<string, number>;
};
type CommandDetectorMap = Record<string, () => Promise<boolean>>;
type CommandDetectionCase = Required<Pick<PlatformDetectOptions, 'platform' | 'pathValue'>> & {
  name: string;
  directoryEntries: Record<string, string[]>;
...
function getVsCodeUserDataDir(homeDir: string): string {
...
function getCommandName(candidate: string): string {
...
async function loadDetectModule(
  t: TestContext,
  { homeDir, existingPaths = [], availableCommands = [] }: LoadDetectOptions
): Promise<LoadedDetect> {
  ...
  const mockReaddir = async (directory: string | Buffer | URL): Promise<string[]> => {
  ...
  const mockAccess = async (candidate: string | Buffer | URL): Promise<void> => {
...
async function loadDetectModuleForPlatform(
  t: TestContext,
  {
    ...
    readdirFailures = [],
    inaccessibleEntries = {},
  }: PlatformDetectOptions
): Promise<LoadedPlatformDetect> {
  const commandChecks: string[] = [];
  const readdirStarts: Map<string, number> = new Map();
...
test('detectors run concurrently when detectMcpClients resolves target table', async (_) => {
  const starts: number[] = [];
  const detectors: CommandDetectorMap = {};
  for (const name of [
    'codex',
    'claude',
    'cursor',
...
test('command detection respects POSIX and Windows PATH delimiters', async (t) => {
  const testCases: CommandDetectionCase[] = [
    {
