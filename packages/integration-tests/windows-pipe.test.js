...
/**
 * Connect a raw socket to the given pipe path.
 *
 * @param {string} pipePath
 * @returns {Promise<net.Socket>}
 */
function connectToPipe(pipePath) {
  ...
  async () => {
    ...
    try {
      ...
      try {
        ...
        assert.equal(/** @type {any} */ (response.result).daemon, 'ok');
        assert.equal(/** @type {any} */ (response.result).extensionConnected, false);
      } finally {
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(/** @type {any} */ (registered).type, 'registered');
      ...
      try {
        ...
        assert.equal(response.ok, true);
        assert.equal(/** @type {any} */ (response.result).url, 'https://pipe-test.example/');
      } finally {
        await client.close().catch(() => {});
        extSocket.destroy();
      }
