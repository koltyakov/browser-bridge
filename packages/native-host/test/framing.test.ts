...
type SocketEmitter = net.Socket & EventEmitter;
...
function createSizedMessage(targetBytes: number): { data: string } {
...
test('createNativeMessageReader parses a max-size frame from many small chunks without concat churn', async (t) => {
  ...
  t.mock.method(Buffer, 'concat', ((list: readonly Uint8Array[], totalLength?: number) => {
    concatCalls += 1;
...
test('createNativeMessageReader fuzzes deterministic random chunk splits for valid and malformed frames', async () => {
  ...
  for (let iteration = 0; iteration < 40; iteration += 1) {
    ...
    const protocolErrors: Error[] = [];
    const frames: Buffer[] = [];
    const expectedMessages: Record<string, unknown>[] = [];
...
test('createNativeMessageReader rejects oversized frames and destroys the stream', async () => {
  ...
  const readable = input as unknown as ReadableEmitter;
  readable.destroy = () => {
    destroyCalls += 1;
  };
...
test('writeJsonLine preserves write order under sustained backpressure', async () => {
  ...
  const typedSocket = socket as unknown as SocketEmitter;
  typedSocket.write = (chunk: string | Uint8Array): boolean => {
