...
function createHarness(overrides: { acquireError?: Error } = {}) {
...
test('requestPaused with a continue rule forwards header overrides', async () => {
  const { interceptor, sent, filters } = createHarness();
  await interceptor.addRule(1, {
