...
type PopupTargets = Parameters<typeof renderPopupViewState>[1];
...
test('renderPopupViewState applies the derived popup copy and button state to DOM targets', () => {
  ...
  const targets = {
    accessEyebrow: { textContent: '' } as HTMLElement,
    accessDetail: { textContent: '' } as HTMLElement,
    accessDisclosure: { hidden: true } as HTMLElement,
    controlCard: {
      classList: {
...
test('renderPopupNativeIndicator updates indicator metadata and ignores missing elements', () => {
  const indicator = {
    ...
    title: '',
    ariaLabel: '',
    setAttribute(this: { ariaLabel: string }, name: string, value: string) {
      if (name === 'aria-label') {
        this.ariaLabel = value;
      }
    },
  } as unknown as HTMLSpanElement & { ariaLabel: string };
