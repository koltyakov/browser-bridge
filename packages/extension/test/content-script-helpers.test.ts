...
type TextNodeLike = { nodeType: number; textContent: string | null };
type ElementTextLike = {
...
type RoleElementLike = {
  tagName: string;
  hasAttribute: (name: string) => boolean;
};
type RegistryElement = object;
type WaitElement = {
  id: string;
...
type Helpers = {
  ...
  applyBudget: (budget: HelperBudget) => Required<HelperBudget>;
  escapeTailwindSelector: (selector: string) => string;
  NON_TEXT_INPUT_TYPES: Set<string>;
  getInputImplicitRole: (element: { type: string }) => string | null;
  getImplicitRole: (element: RoleElementLike) => string | null;
  getImplicitRoleSelector: (role: string) => string;
  toRect: (rect: RectLike) => RectLike;
  extractElementText: (element: ElementTextLike) => string;
  pruneElementRegistryEntries: (options: {
    registry: Map<string, RegistryElement>;
    reverseRegistry: WeakMap<RegistryElement, string>;
    iterator: IterableIterator<[string, RegistryElement]> | null;
    containsElement: (element: RegistryElement) => boolean;
    batchSize: number;
  }) => { pruned: boolean; iterator: IterableIterator<[string, RegistryElement]> | null };
  findElementForWaitState: (options: {
    elements: WaitElement[];
    waitState: 'visible' | 'hidden';
    getRect: (element: WaitElement) => { width: number; height: number };
    getVisibility: (element: WaitElement) => string;
  }) => WaitElement | null;
};

const loadedHelpers = Reflect.get(globalThis, '__BBX_CONTENT_HELPERS__') as Helpers | undefined;
assert.ok(loadedHelpers, 'content-script helpers should be registered on globalThis');
const helpers: Helpers = loadedHelpers;
...
test('content script helpers convert rects and extract element text', () => {
  ...
  Reflect.set(globalThis, 'window', { scrollX: 12, scrollY: 34 });
...
test('content script helpers prune only detached element registry entries', () => {
  ...
  const result = helpers.pruneElementRegistryEntries({
    ...
    reverseRegistry,
    iterator: null,
    containsElement: (element: RegistryElement) => attachedElements.has(element),
    batchSize: 10,
  });
...
test('content script helpers visibility matching avoids duplicate layout reads', () => {
  const visibleElements: WaitElement[] = [
  ...
  function runCase(elements: WaitElement[], waitState: 'visible' | 'hidden') {
    const rectCalls = new Map<string, number>();
    const styleCalls = new Map<string, number>();
    const match = helpers.findElementForWaitState({
      elements,
      waitState,
      getRect(element: WaitElement) {
        rectCalls.set(element.id, (rectCalls.get(element.id) ?? 0) + 1);
        return element.rect;
      },
      getVisibility(element: WaitElement) {
