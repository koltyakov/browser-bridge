...
type NavigationTarget = { tabId: number; windowId: number; title: string; url: string };
type UpdateProperties = { url: string };
type NavigationCalls = {
  update: Array<{ tabId: number; properties: UpdateProperties }>;
  reload: number[];
  goBack: number[];
  goForward: number[];
...
type NavigationDependencies = {
  ...
  updateTab: (tabId: number, properties: UpdateProperties) => Promise<unknown>;
...
function createDependencies(
  overrides: Partial<NavigationDependencies> = {}
...
test('handleNavigationRequest rejects unsupported navigation URL schemes', async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      const bridgeError = error as Error & { code?: string; message?: string };
