...
type AccessChrome = Parameters<typeof getAccessStatus>[0]['chrome'];
...
test('getAccessStatus reports no_routable_active_tab when the active tab lacks a routable target', async () => {
  const queries: Array<{ active: boolean; windowId: number }> = [];
  const status = await getAccessStatus({
    chrome: {
      windows: {
      ...
      tabs: {
        async query(query: { active: boolean; windowId: number }) {
          queries.push(query);
