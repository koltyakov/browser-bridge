...
type TabQuery = { windowId: number };
type TabCreateProperties = { url: string; active: boolean; windowId: number };
type BridgeErrorLike = Error & { code?: string };
...
test('handleCreateTab rejects invalid tab URLs before calling chrome.tabs.create', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      ...
      const bridgeError = error as BridgeErrorLike;
