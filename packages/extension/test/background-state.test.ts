...
type BackgroundState = {
  ...
  enabledWindow: EnabledWindow | null;
...
type BackgroundModule = Record<string, unknown> & {
  getStateForTest: () => BackgroundState;
  ...
  isWindowEnabled: (windowId: number) => boolean;
  isAccessRequestedWindow: (windowId: number) => boolean;
  clearRequestedAccessWindow: (windowId?: number) => void;
  clearRequestedAccessPopupWindow: (windowId?: number) => void;
  isTabEnabled: (tabId: number) => Promise<boolean>;
  isAccessRequestedTab: (tabId: number) => Promise<boolean>;
  updateActionIndicatorForTab: (tabId: number) => Promise<void>;
};

type StateChrome = ChromeFake & {
...
type LoadedStateBackground = {
  chrome: StateChrome;
  module: BackgroundModule;
};

type ExecuteScriptCall = chrome.scripting.ScriptInjection<unknown[], unknown> & {
  args?: unknown[];
};
...
async function loadStateBackground(
  options: LoadBackgroundOptions = {}
): Promise<LoadedStateBackground> {
  return (await loadBackground(options)) as unknown as LoadedStateBackground;
}

function createNativePort(messages: unknown[]): FakeRuntimePort {
  return {
...
function createBufferedExecuteScript(
  consoleBuffers: Map<number, BufferState>,
  networkBuffers: Map<number, BufferState>,
  executeScriptCalls: ExecuteScriptCall[]
): (details: ExecuteScriptCall) => Promise<Array<{ result: unknown }>> {
  return async function executeScript(details: ExecuteScriptCall) {
...
test('background state scope.set_enabled enables and disables the requested current window', async () => {
  ...
  const alarmCreates: Record<string, unknown>[] = [];
  ...
  const loaded = await loadStateBackground({
    chrome,
...
test('background state rollbackAllPatchesForTab only rolls back tracked patch ids', async () => {
  const sentMessages: Array<{ tabId: number; message: BridgeMessage }> = [];
  const rollbackPatchIds: string[] = [];
  const chrome = createChromeFake({
    tabs: {
      async sendMessage(tabId: number, message: BridgeMessage) {
...
test('background state clearTabBridgeState swallows recoverable instrumentation errors', async () => {
  ...
  const chrome = createChromeFake({
    tabs: {
      async sendMessage(_tabId: number, message: BridgeMessage) {
        if (message.type === 'bridge.ping') {
          return { ok: true };
        }
    ...
    scripting: {
      async executeScript(details: ExecuteScriptCall) {
        executeScriptCalls.push(details);
        const source = String(details.func);
        if (source.includes('__bb_console_buffer')) {
...
test('background state persists enabled-window title updates and refreshes action state', async () => {
  const executeScriptCalls: ExecuteScriptCall[] = [];
  const badgeTextCalls: ActionDetails[] = [];
  const titleCalls: ActionDetails[] = [];
  const chrome = createChromeFake({
    tabs: {
      async get(tabId: number) {
    ...
    action: {
      async setBadgeText(details: ActionDetails) {
        badgeTextCalls.push(details);
      },
      async setTitle(details: ActionDetails) {
...
test('background state updateActionIndicatorForTab marks enabled tabs', async () => {
  const badgeBackgroundCalls: ActionDetails[] = [];
  const badgeTextColorCalls: ActionDetails[] = [];
  const badgeTextCalls: ActionDetails[] = [];
  const titleCalls: ActionDetails[] = [];
  const chrome = createChromeFake({
    tabs: {
    ...
    action: {
      async setBadgeBackgroundColor(details: ActionDetails) {
        badgeBackgroundCalls.push(details);
      },
      async setBadgeTextColor(details: ActionDetails) {
