...
type PopupPort = ReturnType<Parameters<typeof connectPopupPort>[0]['connect']>;
type PopupRenderCall = [string, ...(boolean | string | null)[]];
...
test('popup runtime connects the popup port and posts a scoped state request', async () => {
  ...
  const connection = await connectPopupPort({
    ...
    connect: (connectInfo) => {
      ...
      return portPair.left.port as unknown as PopupPort;
    },
    onMessage,
  });
