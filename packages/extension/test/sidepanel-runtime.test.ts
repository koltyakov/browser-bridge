...
type SidepanelPort = ReturnType<Parameters<typeof connectSidepanelPort>[0]['connect']>;
type SidepanelRenderCall = [string, ...unknown[]];
type SidepanelState = Parameters<typeof renderSidepanelState>[0];
type ActionLogEntry = SidepanelState['actionLog'][number];
...
test('sidepanel runtime connects the port, posts a state request, and schedules reconnects', () => {
  const connectCalls: chrome.runtime.ConnectInfo[] = [];
  const reconnects: Array<{ callback: () => void; delayMs: number }> = [];
  ...
  const connection = connectSidepanelPort({
    connect: (connectInfo) => {
      ...
      return portPair.left.port as unknown as SidepanelPort;
    },
    onMessage,
...
test('sidepanel runtime message handler routes native status, sync, and toggle errors', () => {
  const calls: SidepanelRenderCall[] = [];
  const state = {
...
test('sidepanel runtime orchestrates full state rendering and collapses examples when activity exists', () => {
  ...
  renderSidepanelState(state, {
    ...
    renderActionLogEntry: (nextEntry, setupStatus, entries, index) => {
      ...
      return { id: nextEntry.id } as unknown as HTMLElement;
    },
