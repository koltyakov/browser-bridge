 ...
 * @param {boolean} clear
 * @param {ChromeWithScripting} chromeObj
 * @returns {Promise<{ entries: Array<{method: string, url: string, status: number, duration: number, type: string, ts: number, size: number}>, dropped: number, provenance: 'page_main_world', integrity: 'untrusted' }>}
 */
export async function readNetworkBuffer(tabId, clear, chromeObj) {
  ...
  return {
    entries: Array.isArray(result.entries)
      ...
      : [],
    dropped: normalizeNonNegativeInteger(result.dropped, 1_000_000),
    provenance: 'page_main_world',
    integrity: 'untrusted',
  };
}

/** @param {unknown} value @param {number} [maximum=Number.MAX_SAFE_INTEGER] @returns {number} */
...
/**
 * MAIN-world state is controlled by the inspected page. Copy only the bounded
 * fields this feature owns before returning evidence to an agent.
 *
 * @param {unknown} value
 * @returns {{ method: string, url: string, status: number, duration: number, type: string, ts: number, size: number } | null}
 */
function normalizeNetworkEntry(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const entry = /** @type {Record<string, unknown>} */ (value);
  return {
    method: typeof entry.method === 'string' ? entry.method.slice(0, 32) : '',
    url: sanitizeIncidentalUrl(entry.url),
    status: normalizeNonNegativeInteger(entry.status, 999),
    duration: normalizeNonNegativeInteger(entry.duration),
    type: entry.type === 'xhr' ? 'xhr' : 'fetch',
    ts: normalizeNonNegativeInteger(entry.ts),
    size: normalizeNonNegativeInteger(entry.size),
  };
}
